-- ==========================================
-- REFINISH STEEL: INORGANIC COMPARATOR
-- Purpose: Exhaustively compares any number of inorganic materials
-- based strictly on verified C++ memory paths.
-- Dumps directly to file. Does not spam console.
-- Usage: refinish-compare-inorganic IRON BRASS 58 COPPER
-- ==========================================

local args = {...}

if #args < 2 then
    qerror("You must provide at least two inorganic IDs or indices to compare. Example: refinish-compare-inorganic IRON BRASS")
end

-- ==========================================
-- 1. IDENTIFICATION & EXTRACTION (NO OMISSIONS)
-- ==========================================
...
-- This is the EXACT, exhaustive map from the read script. Nothing omitted.
local function get_inorganic_data(target)
    ...
    if mat.material then
        ...
        if m.state_color then data.material.state_color = { Solid = m.state_color.Solid, Liquid = m.state_color.Liquid, Gas = m.state_color.Gas, Powder = m.state_color.Powder, Paste = m.state_color.Paste, Pressed = m.state_color.Pressed } end
        if m.state_name then data.material.state_name = { Solid = m.state_name.Solid, Liquid = m.state_name.Liquid, Gas = m.state_name.Gas, Powder = m.state_name.Powder, Paste = m.state_name.Paste, Pressed = m.state_name.Pressed } end
        if m.state_adj then data.material.state_adj = { Solid = m.state_adj.Solid, Liquid = m.state_adj.Liquid, Gas = m.state_adj.Gas, Powder = m.state_adj.Powder, Paste = m.state_adj.Paste, Pressed = m.state_adj.Pressed } end

        if m.strength then
            data.material.strength.absorption = m.strength.absorption
            data.material.strength.max_edge = m.strength.max_edge
            if m.strength.yield then data.material.strength.yield = { BENDING = m.strength.yield.BENDING, SHEAR = m.strength.yield.SHEAR, TORSION = m.strength.yield.TORSION, IMPACT = m.strength.yield.IMPACT, TENSILE = m.strength.yield.TENSILE, COMPRESSIVE = m.strength.yield.COMPRESSIVE } end
            if m.strength.fracture then data.material.strength.fracture = { BENDING = m.strength.fracture.BENDING, SHEAR = m.strength.fracture.SHEAR, TORSION = m.strength.fracture.TORSION, IMPACT = m.strength.fracture.IMPACT, TENSILE = m.strength.fracture.TENSILE, COMPRESSIVE = m.strength.fracture.COMPRESSIVE } end
            if m.strength.strain_at_yield then data.material.strength.strain_at_yield = { BENDING = m.strength.strain_at_yield.BENDING, SHEAR = m.strength.strain_at_yield.SHEAR, TORSION = m.strength.strain_at_yield.TORSION, IMPACT = m.strength.strain_at_yield.IMPACT, TENSILE = m.strength.strain_at_yield.TENSILE, COMPRESSIVE = m.strength.strain_at_yield.COMPRESSIVE } end
        end
...
-- ==========================================
-- 2. FLATTENING & COMPARISON LOGIC
-- ==========================================

-- Recursively flattens nested tables into a 1D key-value map.
-- e.g., { material = { flags = { IS_METAL = true } } } -> ["material.flags.IS_METAL"] = true
local function flatten_table(t, prefix, result)
    result = result or {}
    ...
    for k, v in pairs(t) do
        if type(v) == "table" then
            if next(v) == nil then
                result[prefix .. tostring(k)] = "EMPTY"
            else
                flatten_table(v, prefix .. tostring(k), result)
            end
        else
            result[prefix .. tostring(k)] = tostring(v)
        end
    end
    return result
end

-- Gather data for all targeted materials
local flat_materials = {}
local material_ids = {}

for _, target in ipairs(args) do
    local data = get_inorganic_data(target)
    if data then
        local id = data.id
        -- Remove the ID and index from the comparison itself
        data.id = nil
        data.index = nil
        
        table.insert(material_ids, id)
        table.insert(flat_materials, { id = id, map = flatten_table(data) })
    else
        print("WARNING: Could not find or extract data for inorganic: " .. target)
    end
end

if #flat_materials < 2 then
    qerror("Failed to extract enough valid inorganics to run a comparison.")
end

-- Master list of all unique paths found across all targets
local all_keys_set = {}
for _, mat_data in ipairs(flat_materials) do
    for k, _ in pairs(mat_data.map) do
        all_keys_set[k] = true
    end
end

local all_keys = {}
for k, _ in pairs(all_keys_set) do table.insert(all_keys, k) end
table.sort(all_keys)
...
for _, k in ipairs(all_keys) do
    local is_same = true
    local first_val = flat_materials[1].map[k] or "UNBOUND_OR_NIL"
    
    -- Check if every material has the exact same value for this key
    for i = 2, #flat_materials do
        local check_val = flat_materials[i].map[k] or "UNBOUND_OR_NIL"
        if check_val ~= first_val then
            is_same = false
            break
        end
    end
    ...
    if is_same then
        table.insert(similarities, string.format("%s = %s", k, first_val))
    else
        local diff_str = k .. ":"
        for _, mat_data in ipairs(flat_materials) do
            local val = mat_data.map[k] or "UNBOUND_OR_NIL"
            diff_str = diff_str .. string.format("\n\t%s = %s", mat_data.id, val)
        end
        table.insert(differences, diff_str)
    end
end
...
table.insert(out_lines, "REFINISH STEEL: INORGANIC COMPARATOR DUMP")
table.insert(out_lines, "Comparing: " .. table.concat(material_ids, ", "))
table.insert(out_lines, "==========================================")

table.insert(out_lines, "\n=== SIMILARITIES ===")
for _, s in ipairs(similarities) do
    table.insert(out_lines, s)
end

table.insert(out_lines, "\n=== DIFFERENCES ===")
for _, d in ipairs(differences) do
    table.insert(out_lines, d)
end

local filename = "refinish_compare_dump.txt"
local file = io.open(filename, "w")
if file then
    file:write(table.concat(out_lines, "\n"))
    file:close()
    print("SUCCESS: Exhaustive comparison completed. Data dumped to " .. filename)
else
    qerror("FAILED: Could not write to " .. filename)
end