...
-- ==========================================
-- DIAGNOSTIC: CIVILIZATION METALLURGY PROBE
-- ==========================================
...
function run()
    ...
    local tech_map = {} -- Will store { [metal_id] = "Source description" }
    ...
    if player_civ.resources and player_civ.resources.metals then
        for _, mat_idx in ipairs(player_civ.resources.metals) do
            ...
            if mat and mat.material.flags.IS_METAL then
                -- Only record it if we haven't found it yet, or append the source
    ...
    -- ==========================================
    -- If a civ can mine an ore, they implicitly know how to smelt its metal.
    if player_civ.resources and player_civ.resources.minerals then
        for _, min_idx in ipairs(player_civ.resources.minerals) do
            ...
            if mineral and mineral.metal_ore then
                -- metal_ore is a vector mapping the ore to the metals it produces
    ...
    -- ==========================================
    -- Dissecting permitted reactions to see if they output metal bars
    ...
    for _, pid in ipairs(permitted_rxns) do
        ...
        if rxn then
            for _, prod in ipairs(rxn.products) do
                -- We only care about products that are physical ITEMS, specifically BARS
                if df.reaction_product_itemst:is_instance(prod) and prod.item_type == df.item_type.BAR then
                    
                    -- INORGANIC mats have mat_type 0. 
                    ...
                    -- We log non-0 mat_types just in case Adamantine is hiding as a bizarre mat_type
